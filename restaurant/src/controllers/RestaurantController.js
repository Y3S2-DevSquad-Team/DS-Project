...
const getRestaurant = async (req, res) => {
  try {
    const restaurant = await Restaurant.findById(req.params.id).populate("menuItems"); // This will include full menu item documents
...
const updateRestaurant = async (req, res) => {
  try {
    const { id } = req.params;
    const { name, description, address, contactNumber, openingHours, availability, ownerId } = req.body;
...
const completeOrder = async (req, res) => {
  try {
    ...
    restaurant.currentOrders = restaurant.currentOrders.filter((id) => id.toString() !== orderId);
